...
sortin 完成后输出的摘要（脚本预览截断至 20 字符，Agent 输出可展示全文）：
...
1. 用户明确指定维度（`| 待归入:dim` 后缀）→ 按用户指定归类，即使语义不完全匹配
2. 精确匹配已有维度 → 追加到该维度文件
3. 语义相近维度 → 追加到最相近维度，标注"相关"
4. 无匹配维度 → 追加到 `general.md`（通用兜底）

> **实现说明**：`sortin.py` 脚本仅实现步骤 1（解析 `| 待归入:dim` 后缀）和步骤 4（默认 `general`）。步骤 2-3 的语义匹配由 Agent 在调用脚本前完成——Agent 分析 raw 条目内容后写入 `| 待归入:dim` 后缀，再由脚本执行机械性追加。`resort` 硬维护的去重、归类、合并同样由 Agent 语义分析完成。
